...
from export_Gen import Expot as Decoder
# import argparse
...
def get_model(model_save_dir, epoch , config, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
...
# @click.option('--config',
#             type=click.dict,
#             default={'con1': 8, 'con2': 4, 'con3': 4, 'con4': 4, 'con5': 4,
...
def test(name, gpu_id, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    config = {'con1': 8, 'con2': 4, 'con3': 4, 'con4': 4, 'con5': 4,
                'RCAB1': [8, 8], 'decon4': [4, 8], 'RCAB2': [8, 16], 'decon5': [4, 4],
                'RCAB3': [12, 16], 'decon6': [4, 4], 'RCAB4': [12, 16], 'decon7': [8, 8], 'decon8': [8, 8]}
    # Load the trained generator.    
    model_save_dir = '/home/lizl/snap/third-stage/Export/finute/dis_result_train/models'
    print_log = open(os.path.join('/home/lizl/snap/third-stage/all-evaluate/finute', name + 'getbest.txt'), 'w')
    print(print_log)
    ...
    for k in range(begin_epoch, end_epoch, step):
        ...
        generator = get_model(model_save_dir, epoch=k, config=config) 
        psnr_score = []
        ssim_score = []
        uiqm_score = []
        uciqe_score = []
        ...
        print('epoch[{}]:---flops: {}---params: {}---Psnr_score: {}---Ssim_score:{}---uiqm_va: {}---uciqe_va:{}'.format(k, flops, params, psnr_va, ssim_va, uiqm_va, uciqe_va))
        print('epoch[{}]:---flops: {}---params: {}---Psnr_score: {}---Ssim_score:{}---uiqm_va: {}---uciqe_va:{}'.format(k, flops, params, psnr_va, ssim_va, uiqm_va, uciqe_va), file=print_log)
