...
def test(name, ngf, gpu_id, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    ...
    print_log = open(os.path.join('/home/lizl/snap/third-stage/evaluate-NoDWT', name + 'getbest.txt'), 'w')
    print(print_log)
    ...
    for k in range(begin_epoch, end_epoch, step):
        ...
        generator.configs = {'con1': 8, 'con2': 4, 'con3': 4, 'con4': 4, 'con5': 4, 'RCAB1': [8, 8], 'decon4': [4, 8], 'RCAB2': [8, 16], 'decon5': [4, 4], 'RCAB3': [12, 16], 'decon6': [4, 4], 'RCAB4': [12, 16], 'decon7': [8, 8], 'decon8': [8, 8]}
        psnr_score = []
        ssim_score = []
        uiqm_score = []
        uciqe_score = []
