...
#----------------------------Loss_lambda_relate---------------------------
    ...
    def calc_distill_loss(self):  # 计算蒸馏损失
        ...
        for i, netA in enumerate(self.netAs):  # netAs是用来学习这些映射的
            ...
            # netA_replicas = replicate(netA, self.gup_id)  # ready gup_id必须是一个列表 # replicate()函数有什么用？  http://t.csdn.cn/6hemq 好像是与并行计算有关
            ...
            n = self.mapping_layers[i]
            Sacts = netA(tuple([self.Sacts[key] for key in sorted(self.Sacts.keys()) if n in key])[0])
            Tacts = [self.Tacts[key] for key in sorted(self.Tacts.keys()) if n in key][0]  # sorted 作用，这一步应该是取参数
            loss = F.l1_loss(Sacts, Tacts)  # zip作用，将Sacts和Tacts对应的索引组合在一个元组里
            KD_dwtloss.append(self.global_dwt_loss(Sacts, Tacts))
            # loss = gather(loss, self.gup_id).sum()  # gather作用，对于单GPU来说，gather后的loss由list就变成了单个值
    ...
    def backward_G(self, k):
        ...
        g_loss = g_loss_gan + global_loss_l1 + angular_loss + distill_loss + KD_dwtloss
        g_loss.backward()
    ...
    def train(self):
        ...
        elif self.dis_train_teacher_epoch:
