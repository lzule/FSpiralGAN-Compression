...
from nets.export_Gen import Expot as Student_G
...
#----------------------------Loss_lambda_relate---------------------------
    ...
    def restore_Dis_model(self):  # 含有教师的鉴别器
        # student_model
        student_G_path = os.path.join(self.student_dir, 'export-146-global_G.ckpt')
    ...
    def build_model(self):
        ...
        self.student_G = Student_G(config=config)
        ...
        for i, name in enumerate(self.mapping_layers):
            ft = self.teacher_global_g_dim
            netA = nn.Conv2d(in_channels=self.fs[i], out_channels=ft, kernel_size=1).to(self.device)
            self.init_net(netA)
